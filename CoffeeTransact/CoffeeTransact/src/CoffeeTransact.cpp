...
public:
  ...
  inline bool validateAndPerform(const char *iUserId) const
  {
    ...
    {
      Statement(mSqlite, std::string("INSERT INTO transactions SELECT datetime('now', 'localtime'), id, -").append(std::to_string(wUnitCostCents)).append(" FROM members WHERE userid = \"").append(iUserId).append("\";")).runOnce();
      return true;
    }
    return false;
  }
...
private:
  inline int getUnitCostCents() const
  {
    ...
    {
      throw std::runtime_error("Parameter unit_cost not found.");
    }
    return wUnitCost;
  }
